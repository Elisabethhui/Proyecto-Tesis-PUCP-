...
RotationalSymmetry::RotationalSymmetry(QObject* parent) : QThread(parent)
{
...
SimpleMesh::Mesh* RotationalSymmetry::computeCompletion(){
    ...
    cout << "RotationalSymmetry:" << prop->getProperty("performOpenVDB") << endl;
    //cout << "Prop:" << prop << endl;
    Harris3D::Mesh* resultOpenVDB = NULL;
    ...
    if(performOpenVDB){
        ...
        emit progress(20, QString("Integrating volume"));
        resultOpenVDB = applyOpenVDB();
    }

    cout << resultOpenVDB->get_number_vertices() << endl;
    Harris3D::Mesh* resultDecimation = NULL;
    ...
    if(performSimplification){
        ...
        emit progress(40, QString("Simplifying surface"));
        if(performOpenVDB)
    ...
    if(performOpenVDB || performSimplification){
        ...
        if(input->has_color()){
            emit progress(60, QString("Transferring classification"));
        ...
        emit progress(75, QString("Detecting rotational symmetry"));
        applyRANSAC(resultClass, center, directionVector);

        std::cout << "Alignment" << std::endl;
        emit progress(90, QString("Aligning surfaces"));
        //output = alignMeshes(resultPrepro, center, directionVector);
        output = alignMeshes(input, center, directionVector);
        ...
        if(input->has_color())
            delete resultClass;
        emit progress(100, QString("Done"));

        return output;
}

void RotationalSymmetry::run(){
    cout << "RotationalSymmetry::run" << endl;
    computeCompletion();
}
