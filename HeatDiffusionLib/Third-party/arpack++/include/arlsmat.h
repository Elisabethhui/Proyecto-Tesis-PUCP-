    ...
    if (j<=k) {
      if (uplo == 'U') {
        if ((irow[j]<0)||(irow[k]>i)){
            cout << "Segundo check" << i << endl;
            return false;
        }
      }
      else { // uplo == 'L'.
        if ((irow[j]<i)||(irow[k]>=this->n)){
            cout << "Tercer check" << i << " " << this->n << endl;
            return false;
        }
      }
      while ((j!=k)&&(irow[j]<irow[j+1])) j++;
      if (j!=k) {
        cout << "Cuarto check" << i << endl;
        return false;
      }
    }
  }
...
void ARluSymMatrix<ARTYPE>::FactorA()
{
  ...
  // Quitting the function if A was not defined.
	//cout << "ARluSymMatrix::FactorA" << endl;
  if (!this->IsDefined()) {
