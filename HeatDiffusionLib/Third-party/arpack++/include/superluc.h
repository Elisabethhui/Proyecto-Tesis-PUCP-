...
{
	//cout << "gstrf:" << A->Dtype <<  endl;
  if (A->Dtype == SLU_D) {       // calling the double precision routine.
	  //cout << "P1->" <<perm_c << endl;
		dgstrf(options,A, relax, panel_size,etree,work,lwork,perm_c,perm_r,L,U,stat,info);
  }
  else if (A->Dtype == SLU_S) {  // calling the single precision routine.
    sgstrf(options,A, relax, panel_size,etree,work,lwork,perm_c,perm_r,L,U,stat,info);
  }
