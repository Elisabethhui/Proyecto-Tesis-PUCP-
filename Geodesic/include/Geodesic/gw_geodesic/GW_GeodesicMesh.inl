...
:	GW_Mesh(),
	...
	bIsMarchingBegin_			( GW_False ),
        ...
        L                                       (NULL),
        dmax                                    (1e9),
        stopCriterion                           (GW_False),
        end_points                              (NULL),
        nend                                    (-1),
        nbr_iter                                (0),
        niter_max                               (-1),
        vertexInsertionCriterion                (GW_False)
{
	/* NOTHING */
}
...
/*------------------------------------------------------------------------------*/
GW_INLINE
GW_Float GW_GeodesicMesh::BasicWeightCallback(GW_GeodesicVertex& Vert)
{
	return 1;
}
...
GW_INLINE
void GW_GeodesicMesh::setStopInformation(GW_U32 *ep, GW_U32 n, GW_Float d){
    stopCriterion = GW_True;
    end_points = ep;
    nend = n;
    dmax = d;
}
...
GW_INLINE
GW_Bool GW_GeodesicMesh::checkStopCriterion(GW_GeodesicVertex &Vert){
    GW_U32 i = Vert.GetID();
    if(Vert.GetDistance() > dmax)
        return GW_True;

    for(int k = 0; k < nend; i++)
        if(end_points[k]==i)
            return GW_True;
    return GW_False;
}
...
GW_INLINE
void GW_GeodesicMesh::setInsertionConstraint(GW_Float *dist, GW_U32 n){
    vertexInsertionCriterion = GW_True;
    L = dist;
    niter_max = n;
}

GW_INLINE
GW_Bool GW_GeodesicMesh::checkInsertionConstraint(GW_GeodesicVertex &Vert, GW_Float rNewDist)
{
    GW_U32 i = Vert.GetID();
    GW_Bool doinsertion = nbr_iter <= niter_max;
    if(L!=NULL)
        doinsertion = doinsertion && (rNewDist < L[i]);
    nbr_iter++;
    return doinsertion;
}

GW_INLINE
void GW_GeodesicMesh::setWeights(GW_Float *w){
    W = w;
}
/*------------------------------------------------------------------------------*/
...
//	std::make_heap( ActiveVertex_.begin(), ActiveVertex_.end(), GW_GeodesicVertex::CompareVertex );
	...
	GW_GeodesicVertex* pCurVert = ActiveVertex_.front();
	GW_ASSERT( pCurVert!=NULL );
	std::pop_heap( ActiveVertex_.begin(), ActiveVertex_.end(), GW_GeodesicVertex::CompareVertex );
...
#if 0	// just for debug
	for( IT_GeodesicVertexVector it = ActiveVertex_.begin(); it!=ActiveVertex_.end(); ++it )
		GW_ASSERT( pCurVert->GetDistance()<=(*it)->GetDistance() );
#endif
		...
		else
		{
			...
			case GW_GeodesicVertex::kFar:
				/* ask to the callback if we should update this vertex and add it to the path */
                                if( vertexInsertionCriterion==GW_False ||
                                        checkInsertionConstraint(*pNewVert,rNewDistance ) )
				{
					pNewVert->SetDistance( rNewDistance );
					/* add the vertex to the heap */
					ActiveVertex_.push_back( pNewVert );
					std::push_heap( ActiveVertex_.begin(), ActiveVertex_.end(), GW_GeodesicVertex::CompareVertex );
	...
	/* the user can force ending of the algorithm */
        ...
        */
        if(stopCriterion==GW_True && bIsMarchingEnd_==GW_False)
            bIsMarchingEnd_ = checkStopCriterion(*pCurVert);
...
{	
        //GW_Float F = this->WeightCallback_( CurrentVertex );
        GW_U32 i = CurrentVertex.GetID();
        GW_Float F = W[i];

	if( Vert1.GetState()!=GW_GeodesicVertex::kFar ||
		Vert2.GetState()!=GW_GeodesicVertex::kFar )
	{
		...
		GW_Bool bVert1Usable = Vert1.GetState()!=GW_GeodesicVertex::kFar && Vert1.GetFront()==&CurrentFront;
		GW_Bool bVert2Usable = Vert2.GetState()!=GW_GeodesicVertex::kFar && Vert2.GetFront()==&CurrentFront;
		...
		{
#else
		GW_Bool bVert1Usable = Vert1.GetState()==GW_GeodesicVertex::kDead && Vert1.GetFront()==&CurrentFront;
		GW_Bool bVert2Usable = Vert2.GetState()==GW_GeodesicVertex::kDead && Vert2.GetFront()==&CurrentFront;
		if( !bVert1Usable && bVert2Usable )
		{
			/* only one point is a contributor */
			return d2 + a * F;
